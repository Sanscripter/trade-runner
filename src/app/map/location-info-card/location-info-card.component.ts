...
export class LocationInfoCardComponent implements OnChanges {
  ...
  getCurrentDistaceInDays() {
    ...
    const speed = this.player?.currentStats.speed!.value!; //speed of the player
    const distance = Math.sqrt(
      Math.pow(currentPosition.x - this.location!.x, 2) +
        Math.pow(currentPosition.y - this.location!.y, 2)
    );
